...
def get(param, saveto, cmd="rtmpdump"):
    ...
    for i, content in enumerate(contents):
        ...
        try:
            ...
            logging.error("Failed to execute command: " + com)

    if len(contents) <= 1:
        # rename and move
        os.rename(save_tmp, saveto)
    else:
        print("Join the files since this files are multiple")
        # combine files
        ffmpeg = which("ffmpeg")
        ...
        try:
            subprocess.check_call(shlex.split(com))
            print("✓ Finihsed to join files: " + saveto)
        except:
            logging.error("✗ Failed to execute command " + com)
...
def parse():
    ...
    convert.add_argument("-s", "--start", help="start time",
                         default=1800)
    convert.add_argument("-t", help="cutting time", default=1770)
